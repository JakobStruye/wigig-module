...
BurstyApplication::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::BurstyApplication")
    ...
    .AddAttribute ("EnableTimestamp", "Whether we add timestamp tag to each outgoing packet or not.",
                  BooleanValue (false),
                  MakeBooleanAccessor (&BurstyApplication::m_enableTimestamp),
                  MakeBooleanChecker ())
    .AddTraceSource ("TagCreated", "A new tag is created and is sent",
                    MakeTraceSourceAccessor (&BurstyApplication::m_tagCreated),
...
uint64_t
BurstyApplication::GetFragmentsPerBurst (void) const
{
    return std::ceil( (double) m_burstSize / m_fragSize);
}
